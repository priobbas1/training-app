import { useEffect } from "react";
import { Link, Navigate } from "react-router-dom";
import { getWorkoutsList, likeWorkout } from "../http/workouts-api";

async function WorkoutElement() {
  let respuesta;
  useEffect(() => {
    respuesta = getWorkoutsList();
    console.log(respuesta, "respuesta");
  }, []);
  return (
    <li className="workout-list">
      <section className="workout-list">
        <article className="workout-list">
          {
            <img
              className="workout-gif"
              /* src={`${process.env.REACT_APP_BACKEND}uploads/workouts/2/71578b54-5304-4c22-b2f4-25c3f0916e7d.gif`} */
              src={`${respuesta}`}
              title="Farmer's Carry"
              alt="Farmer's Carry workout"
            />
          }
          <h2 className="workout-name">Farmer's Carry</h2>
          <h3 className="workout-muscle">full body workout</h3>
          <Link to="/create">
            <button>CREATE</button>
          </Link>
          <Link onClick={likeWorkout}>
            <button>
              <svg xmlns="http://www.w3.org/2000/svg" height="48" width="48">
                <path d="m16.15 37.75 7.85-4.7 7.85 4.75-2.1-8.9 6.9-6-9.1-.8L24 13.7l-3.55 8.35-9.1.8 6.9 6ZM11.65 44l3.25-14.05L4 20.5l14.4-1.25L24 6l5.6 13.25L44 20.5l-10.9 9.45L36.35 44 24 36.55ZM24 26.25Z" />
              </svg>
            </button>
          </Link>
        </article>
      </section>
    </li>
  );
}

export default WorkoutElement;
